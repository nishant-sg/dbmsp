...
router.post("/drop", async (req, res) => {
  connection.query("drop table if exists Payment", (err) => {
    if (err) {
      console.error(err);
...
router.get("/scheme", async (req, res) => {
  connection.query("describe Payment", (err, result) => {
    if (err) {
      console.error(err);
...
router.post("/selectedrows", async (req, res) => {
  connection.query(
    `select * from Payment where ${req.body["message"]}`,
    (err, result) => {
      if (err) {
        console.error(err);
        return res.status(400).json({ success: false, error: err });
...
router.post("/insert", async (req, res) => {
  connection.query(
    `insert into Payment values (${req.body["message"]})`,
    (err, result) => {
      if (err) {
        console.error(err);
        return res.status(400).json({ success: false, error: err });
...
router.post("/update", async (req, res) => {
  connection.query(`update Payment ${req.body["message"]}`, (err, result) => {
    if (err) {
      console.error(err);
      return res.status(400).json({ success: false, error: err });
    }
...
router.post("/delete", async (req, res) => {
  connection.query(
    `delete from Payment where ${req.body["message"]}`,
    (err, result) => {
      if (err) {
        console.error(err);
        return res.status(400).json({ success: false, error: err });
