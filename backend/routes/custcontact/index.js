...
router.post("/drop", async (req, res) => {
  connection.query("drop table if exists CustContact", (err) => {
    if (err) {
      console.error(err);
...
router.get("/scheme", async (req, res) => {
  connection.query("describe CustContact", (err, result) => {
    if (err) {
      console.error(err);
...
router.post("/allrows", async (req, res) => {
  connection.query("select * from CustContact", (err, result) => {
    if (err) {
      console.error(err);
      return res.status(400).json({ success: false, error: err });
    }
...
router.post("/selectedrows", async (req, res) => {
  connection.query(
    `select * from CustContact where ${req.body["message"]}`,
    (err, result) => {
      if (err) {
        console.error(err);
        return res.status(400).json({ success: false, error: err });
...
router.post("/update", async (req, res) => {
  connection.query(
    `update CustContact ${req.body["message"]}`,
    (err, result) => {
      if (err) {
        console.error(err);
        return res.status(400).json({ success: false, error: err });
...
router.post("/delete", async (req, res) => {
  connection.query(
    `delete from CustContact where ${req.body["message"]}`,
    (err, result) => {
      if (err) {
        console.error(err);
        return res.status(400).json({ success: false, error: err });
